...
<form onsubmit="return set_data()" target="update" style="display:inline;">
  <strong>From:</strong>
  <input id="fst" type="date" value=<?php echo exec("date -d '$interval ago' '+%Y-%m-%d'"); ?>></input>
  <strong>To:</strong>
  <input id="last" type="date" value=<?php echo exec("date '+%Y-%m-%d'"); ?>></input>
  <input id="rep" type="submit" value="Update Graph"></input>
  <iframe name="update" style="display:none;"></iframe>
</form>
...
<script>
  function set_data() {
    ...
    var clus = <?php echo json_encode($cluster); ?>;
    $.ajax({
      type: "POST",
      url: "rldavrg.php",
      data: {
